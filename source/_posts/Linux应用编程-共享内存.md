...
tags: "Linux应用开发\r"
excerpt: <p>Linux应用编程-共享内存 1、共享内存由以下特点： 2、共享内存的优缺点： shmget
  函数的创建或获取一个共享内存对象，并返回共享内存标识符。 shmat 映射函数 ，共享内存区对象映射到调用进程的地址空间 。 shmdt
  函数是用来解除进程与共享内存之间的映射的，在解除映射后，该进程不能再访问这个共享内存 。</p>
---
