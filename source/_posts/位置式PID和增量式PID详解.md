...
tags: "PID\r"
excerpt: <p>位置式PID和增量式PID详解 1. 核心思想
  PID（比例-积分-微分）控制器通过动态调整系统输出，使实际值快速、稳定地跟踪目标值。其核心是 误差驱动 ： - 误差（e t ：目标值与实际值的差值。 -
  控制目标 ：最小化误差的瞬时值、累积值及变化趋势。 2. 三大组成部分 - 比例（P） ：根据当前误差调整输出，误</p>
---
