...
tags: "MQTT\r"
excerpt: <p>mqtt协议解析 TOC - 固定报头（所有控制报文都包含） - 可变报头 - 消息体 - 控制报文的类型 -
  控制报文的类型由第一个字节的4-7位组成 - 控制报文类型标志位 - 剩余长度 ：从第二个字节开始，剩余长度（Remaining Length）
  表示当前报文剩余部分的字节数， 包括可变报头和负载的数据。 剩</p>
---
