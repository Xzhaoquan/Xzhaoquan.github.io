...
function Media({ editor, onUpload, onInsert }: { editor: Document | null; onUpload: (file: File) => void; onInsert: (asset: MediaAsset) => void }) {
  ...
  return <section className="card"><div className="toolbar"><div><h2>Post media assets</h2>{editor ? <p className="muted">Open post: <b>{editor.title}</b></p> : null}</div>{editor?.kind === 'post' && <button onClick={() => void load()}>Refresh</button>}</div>{editor?.kind === 'post' ? <><label className="dropzone">Choose an image or attachment<input type="file" onChange={event => event.target.files?.[0] && onUpload(event.target.files[0])} /></label>{error && <p className="notice">{error}</p>}{assets.length ? <table><thead><tr><th>Preview</th><th>Name</th><th>Size</th><th /></tr></thead><tbody>{assets.map(asset => <tr key={asset.path}><td>{image(asset) ? <a href={mediaUrl(asset)} target="_blank"><img className="media-thumb" src={mediaUrl(asset)} alt={asset.name} /></a> : <span>File</span>}</td><td>{asset.name}<small>{asset.path}</small></td><td>{Math.ceil(asset.size / 1024)} KB</td><td><div className="actions"><a href={mediaUrl(asset)} target="_blank">Open</a><button onClick={() => onInsert(asset)}>Insert</button><button onClick={() => void copy(asset)}>Copy Markdown</button><button className="danger" onClick={() => void remove(asset)}>Delete</button></div></td></tr>)}</tbody></table> : <p className="muted">No media files for this post yet.</p>}</> : <p>Open a published post in the content list first; uploaded media will be stored with that post’s assets.</p>}</section>;
}
...
function Editor({ item, onChange, onSave, onRename, onClose, onUpload }: { item: Document; onChange: (item: Document) => void; onSave: () => void; onRename: (item: Document) => void; onClose: () => void; onUpload: (file: File) => void }) {
  ...
  return <div className="modal"><div className="editor-modal"><div className="modal-head"><div><b>{item.title}</b><small>{item.path}</small></div><div>{item.kind === 'page' && <button onClick={() => onRename(item)}>Change path</button>}<button onClick={onSave}>Save</button><button className="primary" onClick={onClose}>Done</button></div></div><div className="editor-layout"><aside className="properties"><label>Title<input value={String(item.data.title ?? item.title)} onChange={event => setData('title', event.target.value)} /></label>{item.kind !== 'page' && <><label>Categories<input value={arrayField('categories')} onChange={event => setData('categories', event.target.value.split(',').map(value => value.trim()).filter(Boolean))} /></label><label>Tags<input value={arrayField('tags')} onChange={event => setData('tags', event.target.value.split(',').map(value => value.trim()).filter(Boolean))} /></label></>}{item.kind === 'page' && <><label>Layout<input value={String(item.data.layout ?? '')} onChange={event => setData('layout', event.target.value)} /></label><label>Show in navigation<select value={String(item.data.menu ?? 'true')} onChange={event => setData('menu', event.target.value === 'true')}><option value="true">Yes</option><option value="false">No</option></select></label></>}<label>Cover image<input value={String(item.data.cover ?? '')} onChange={event => setData('cover', event.target.value)} /></label><label className="file-button">Upload image<input type="file" accept="image/*" onChange={event => event.target.files?.[0] && onUpload(event.target.files[0])} /></label><small>{characters} characters · {words} words · about {minutes} min read</small><small>Autosaves source after two seconds. Use Ctrl+S to save now.</small></aside><section className="editor-workspace"><div className="editor-tabs"><button className={mode === 'edit' ? 'active' : ''} onClick={() => setMode('edit')}>Edit Markdown</button><button className={mode === 'preview' ? 'active' : ''} onClick={() => setMode('preview')}>Preview</button></div>{mode === 'edit' ? <MarkdownEditor key={item.path} value={item.body} onChange={body => onChange({ ...item, body })} onSave={onSave} /> : <article className="preview"><h3>{String(item.data.title ?? item.title)}</h3><p className="muted">{arrayField('categories')}</p><ReactMarkdown remarkPlugins={[remarkGfm]}>{item.body}</ReactMarkdown></article>}</section></div></div></div>;
}
