...
function App() {
  ...
  const [git, setGit] = useState<{ branch: string; changes: string[]; conflicts: string[]; log: Array<{ hash: string; message: string; date: string }> } | null>(null);
...
function GitView({ data, diff, onDiff, onRefresh, onPull, onCommit, onPush }: { data: { branch: string; changes: string[]; conflicts: string[]; log: Array<{ hash: string; message: string; date: string }> } | null; diff: string; onDiff: (path: string) => void; onRefresh: () => void; onPull: () => void; onCommit: (paths: string[], message: string) => void; onPush: () => void }) {
  ...
  return <section className="grid two"><article className="card"><div className="toolbar"><h2>Working tree · {data?.branch ?? '—'}</h2><button onClick={onRefresh}>Refresh</button></div>{data?.conflicts.length ? <p className="git-conflict">Conflicts detected: {data.conflicts.join(', ')}. Resolve them in an external Git tool before continuing.</p> : null}{data?.changes.length ? data.changes.map(change => { const file = change.slice(3); return <div className="git-change" key={change}><input type="checkbox" checked={selected.includes(file)} onChange={() => toggle(file)} /><code>{change}</code><button onClick={() => onDiff(file)}>View diff</button></div>; }) : <p>No uncommitted changes.</p>}<div className="actions"><button onClick={onPull}>Pull source branch</button><button disabled={!selected.length || Boolean(data?.conflicts.length)} onClick={commit}>Commit selected</button><button className="primary" disabled={Boolean(data?.conflicts.length)} onClick={onPush}>Push source branch</button></div></article><article className="card"><h2>Recent commits</h2>{data?.log.map(item => <div className="list-row" key={item.hash}><code>{item.hash}</code><span>{item.message}</span></div>)}</article>{diff && <article className="card git-diff"><h2>Selected file diff</h2><pre>{diff}</pre></article>}</section>;
}
