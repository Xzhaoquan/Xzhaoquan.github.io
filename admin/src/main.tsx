...
function ContentView({ items, query, onQuery, onOpen, onRecycle, onTransition }: { items: Document[]; query: string; onQuery: (value: string) => void; onOpen: (item: Document) => void; onRecycle: (item: Document) => void; onTransition: (item: Document) => void }) { const [category, setCategory] = useState(''); const [tag, setTag] = useState(''); const values = (item: Document, field: 'categories' | 'tags') => Array.isArray(item.data[field]) ? item.data[field].map(String) : item.data[field] ? [String(item.data[field])] : []; const categories = [...new Set(items.flatMap(item => values(item, 'categories')))].sort(); const tags = [...new Set(items.flatMap(item => values(item, 'tags')))].sort(); const visible = items.filter(item => (!category || values(item, 'categories').includes(category)) && (!tag || values(item, 'tags').includes(tag))); return <section className="card"><div className="toolbar content-filters"><input value={query} onChange={event => onQuery(event.target.value)} placeholder="Search titles" /><select value={category} onChange={event => setCategory(event.target.value)}><option value="">All categories</option>{categories.map(name => <option key={name}>{name}</option>)}</select><select value={tag} onChange={event => setTag(event.target.value)}><option value="">All tags</option>{tags.map(name => <option key={name}>{name}</option>)}</select><span>{visible.length} items</span></div><table><thead><tr><th>Title</th><th>Categories / tags</th><th>Updated</th><th /></tr></thead><tbody>{visible.map(item => <tr key={item.path}><td><button className="text-button" onClick={() => onOpen(item)}>{item.title}</button><small>{item.path}</small></td><td>{[...values(item, 'categories'), ...values(item, 'tags')].map(tag => <span className="chip" key={tag}>{tag}</span>)}</td><td>{new Date(item.mtimeMs).toLocaleString()}</td><td><div className="actions"><button onClick={() => onOpen(item)}>Edit</button>{item.kind !== 'page' && <button onClick={() => onTransition(item)}>{item.kind === 'draft' ? 'Publish' : 'To draft'}</button>}<button className="danger" onClick={() => onRecycle(item)}>Delete</button></div></td></tr>)}</tbody></table></section>; }
