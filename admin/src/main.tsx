...
type SeoIssue = { rule: string; level: 'warning' | 'suggestion'; message: string; suggestion: string; autoFixable?: boolean };
...
type ThemeProfile = { enabled: boolean; articleSelfBlock: boolean; avatar: string; gravatar: string; author: string; author_title: string; author_description: string; location: string };
...
function App() {
  ...
  useEffect(() => {
    ...
    if (view === 'theme') action(async () => { const [next, profile] = await Promise.all([api<{ theme: string; themes: string[]; config: { raw: string } }>('/theme'), api<ThemeProfile>('/theme/profile')]); setThemeInfo({ theme: next.theme, themes: next.themes }); setThemeProfile(profile); setRawConfig(next.config.raw); });
  ...
  return <div className={dark ? 'app-shell dark' : 'app-shell'}>
    ...
    <main className="main"><header><div><h1>{nav.find(item => item[0] === view)?.[1]}</h1><p>{status?.root ?? 'Reading Hexo project...'}</p></div><div className="header-actions"><button onClick={() => setDark(value => !value)}>{dark ? 'Light mode' : 'Dark mode'}</button><button onClick={() => refreshStatus()} disabled={busy}>Refresh</button>{(['posts', 'drafts', 'pages'] as View[]).includes(view) && <button className="primary" onClick={create}>New</button>}</div></header>
      ...
      {view === 'theme' && <ThemeManager info={themeInfo} profile={themeProfile} raw={rawConfig} onChange={setRawConfig} onProfileChange={(field, value) => setThemeProfile(current => current ? { ...current, [field]: value } : current)} onProfileSave={() => { if (!themeProfile || !window.confirm('Save profile settings for the current theme? A backup will be created.')) return; action(async () => { await api('/theme/profile', { method: 'PUT', body: JSON.stringify(themeProfile) }); const config = await api<{ config: { raw: string } }>('/theme'); setRawConfig(config.config.raw); setNotice('Theme profile settings saved.'); }); }} onSave={() => { if (window.confirm('Save and overwrite the current theme YAML configuration? A backup will be created.')) action(async () => { await api('/theme', { method: 'PUT', body: JSON.stringify({ raw: rawConfig }) }); setNotice('Theme YAML validated, backed up, and saved.'); }); }} onSelect={theme => { if (theme === themeInfo?.theme || !window.confirm(`Switch the blog theme to “${theme}”?`)) return; action(async () => { const next = await api<{ current: string; installed: string[] }>('/theme/select', { method: 'POST', body: JSON.stringify({ theme }) }); const [config, profile] = await Promise.all([api<{ theme: string; themes: string[]; config: { raw: string } }>('/theme'), api<ThemeProfile>('/theme/profile')]); setThemeInfo({ theme: next.current, themes: next.installed }); setThemeProfile(profile); setRawConfig(config.config.raw); setNotice(`Theme switched to ${next.current}.`); }); }} />}
...
function ThemeManager({ info, profile, raw, onChange, onSave, onSelect, onProfileChange, onProfileSave }: { info: { theme: string; themes: string[] } | null; profile: ThemeProfile | null; raw: string; onChange: (value: string) => void; onSave: () => void; onSelect: (theme: string) => void; onProfileChange: <K extends keyof ThemeProfile>(field: K, value: ThemeProfile[K]) => void; onProfileSave: () => void }) { return <section className="grid"><article className="card"><div className="toolbar"><div><h2>Installed themes</h2><p className="muted">Current theme: <b>{info?.theme || '—'}</b></p></div></div><div className="theme-list">{info?.themes.map(theme => <button key={theme} className={theme === info?.theme ? 'primary' : ''} onClick={() => onSelect(theme)}>{theme === info?.theme ? `${theme} (current)` : theme}</button>) ?? <p>Loading themes…</p>}</div></article><article className="card"><div className="toolbar"><div><h2>Profile</h2><p className="muted">Controls the theme profile bar. Saved to <code>themes/{info?.theme || '…'}/_config.yml</code>.</p></div><button className="primary" disabled={!profile} onClick={onProfileSave}>Save profile</button></div>{profile ? <div className="config-fields"><label className="checkbox-label"><input type="checkbox" checked={profile.enabled} onChange={event => onProfileChange('enabled', event.target.checked)} />Show profile bar</label><label className="checkbox-label"><input type="checkbox" checked={profile.articleSelfBlock} onChange={event => onProfileChange('articleSelfBlock', event.target.checked)} />Show profile below articles</label><label>Avatar path<input value={profile.avatar} placeholder="images/avatar.jpg" onChange={event => onProfileChange('avatar', event.target.value)} /></label><label>Gravatar email<input value={profile.gravatar} placeholder="Optional; overrides avatar" onChange={event => onProfileChange('gravatar', event.target.value)} /></label><label>Author<input value={profile.author} onChange={event => onProfileChange('author', event.target.value)} /></label><label>Author title<input value={profile.author_title} onChange={event => onProfileChange('author_title', event.target.value)} /></label><label className="config-field-full">Author description<textarea rows={3} value={profile.author_description} onChange={event => onProfileChange('author_description', event.target.value)} /></label><label>Location<input value={profile.location} onChange={event => onProfileChange('location', event.target.value)} /></label></div> : <p>Loading theme profile…</p>}</article><ConfigEditor raw={raw} onChange={onChange} onSave={onSave} /></section>; }
