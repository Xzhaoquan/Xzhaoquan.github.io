...
function SeoChecks({ report, onOpen, onFix }: { report: SeoReport | null; onOpen: (article: SeoReport['articles'][number]) => void; onFix: (article: SeoReport['articles'][number], action: 'summary' | 'date' | 'image-alt') => void }) { const fixable: Record<string, ['summary' | 'date' | 'image-alt', string] | undefined> = { description: ['summary', 'Add summary'], date: ['date', 'Set date'], 'image-alt': ['image-alt', 'Fill image alt text'] }; return <section className="card"><div className="toolbar"><div><h2>SEO checks</h2><p className="muted">Informational only: findings do not block saving, generating, or deployment.</p></div><div><b>{report?.warnings ?? 0} warnings</b> · {report?.suggestions ?? 0} suggestions</div></div>{report?.articles.filter(article => article.issues.length).length ? <table><thead><tr><th>Article</th><th>Findings</th><th /></tr></thead><tbody>{report.articles.filter(article => article.issues.length).map(article => <tr key={`${article.kind}:${article.path}`}><td>{article.title}<small>{article.path}</small></td><td>{article.issues.map(issue => { const fix = fixable[issue.rule]; return <div key={issue.rule}><b>{issue.level}</b>: {issue.message}<small>{issue.suggestion}</small>{fix ? <button className="small-button" onClick={() => onFix(article, fix[0])}>{fix[1]}</button> : null}</div>; })}</td><td><button onClick={() => onOpen(article)}>Open</button></td></tr>)}</tbody></table> : <p className="muted">No SEO findings in current posts and drafts.</p>}</section>; }
