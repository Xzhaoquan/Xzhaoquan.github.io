...
function App() {
  ...
  const runTask = async (type: 'clean' | 'generate' | 'deploy' | 'preview') => { const confirmed = type !== 'deploy' || window.confirm('Deploy will publish generated files to the configured deployment branch (main). Continue?'); if (!confirmed) return; await action(async () => { await api(`/tasks/${type}`, { method: 'POST', body: JSON.stringify({ confirmed, port: 4000 }) }); setNotice(`${type} task started.`); }); };
  ...
  return <div className="app-shell">
    ...
    <main className="main"><header><div><h1>{nav.find(item => item[0] === view)?.[1]}</h1><p>{status?.root ?? 'Reading Hexo project...'}</p></div><div className="header-actions"><button onClick={() => refreshStatus()} disabled={busy}>Refresh</button>{(['posts', 'drafts', 'pages'] as View[]).includes(view) && <button className="primary" onClick={create}>New</button>}</div></header>
      ...
      {view === 'publish' && <Publish status={status} tasks={tasks} onTask={runTask} onStop={() => action(async () => { await api('/preview/stop', { method: 'POST' }); })} />}
...
function Publish({ status, tasks, onTask, onStop }: { status: Status | null; tasks: Task[]; onTask: (type: 'clean' | 'generate' | 'deploy' | 'preview') => void; onStop: () => void }) { return <section className="grid two"><article className="card"><h2>Local preview</h2><p>{status?.preview ? <a href={status.preview.url} target="_blank">Running: {status.preview.url}</a> : 'Not running'}</p>{status?.preview ? <button className="danger" onClick={onStop}>Stop preview</button> : <button className="primary" onClick={() => onTask('preview')}>Start preview</button>}</article><article className="card"><h2>Publishing workflow</h2><p className="muted">Commit source changes to <code>hexo</code>. Hexo deploy publishes generated files to <code>main</code>.</p><div className="actions"><button onClick={() => onTask('clean')}>Clean</button><button onClick={() => onTask('generate')}>Generate</button><button className="danger" onClick={() => onTask('deploy')}>Deploy to main</button></div></article><TaskLogs tasks={tasks} /></section>; }
