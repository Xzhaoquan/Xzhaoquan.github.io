...
app.patch('/api/seo/article', async request => { const body = z.object({ kind: z.enum(['post', 'draft']), path: z.string().min(1), action: z.enum(['summary', 'date', 'image-alt']) }).parse(request.body); return success(await context.fixSeoArticle(body.kind, body.path, body.action)); });
